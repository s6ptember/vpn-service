...
### Палитра и типографика (`v5`)

Приложение фиксирует **одну тёмную палитру** и отдаёт её Telegram через `setHeaderColor`/`setBackgroundColor`. Переключателя тем нет и `--tg-theme-*` не читаются: все поверхности, стекло и акцент подогнаны под `#101828` и под светлой темой клиента рассыпаются. Токены живут в `@theme` в `app.css`, литералов цвета в разметке нет.

| Роль | Токен | Значение |
|---|---|---|
| Страница | `--color-page` | `#101828` |
| Карточка | `--color-surface` | `#1A2233` |
| Вложенная поверхность, поля ввода | `--color-elevated` | `#232D42` |
| Линия | `--color-line` | `#344054` |
| За рамкой телефона (десктоп) | `--color-shell` | `#080D16` |
| Основной текст | `--color-ink` | `#FFFFFF` |
| Вторичный текст | `--color-muted` | `#878EA8` |
| Самый тихий текст | `--color-subtle` | `#667085` |
| Акцент | `--color-accent-600` | `#AB93E1` |
| Текст **на** акценте | `--color-on-accent` | `#101828` |

Акцент один. Ключевое следствие: **`#AB93E1` — светлый цвет на тёмной странице**, поэтому всё, что залито акцентом (кнопка `primary`, пилюля островка, карточка `tone="accent"`), несёт `text-on-accent`, а не белый. Инверсия — та же, что на референсных экранах у залитых карточек.

Ряд `accent-100…700` идёт от тёмного к светлому: `100` — подложка под бейдж на тёмном фоне, `700` — акцентный текст. Состояния (`danger`, `warn`) устроены так же: тёмная заливка, светлый текст.

**Один прожектор на экран.** Акцентом заливается ровно одна карточка: на Главной — «Текущий план» при активной подписке, в колоде тарифов — карточка с лучшей ценой за день. Колода залитых карточек иерархии не даёт.

Шкала кеглей — девять шагов, названы по роли, а не по размеру: `text-display` 35 · `text-title` 31 · `text-h1` 27 · `text-h2` 21 · `text-h3` 19 · `text-body` 17 · `text-sm` 15 · `text-xs` 12 · `text-2xs` 11. Произвольных `text-[14px]` в разметке не бывает — экран выбирает шаг.

Гарнитура — **Onest**, вариативная, лежит в `static/fonts` по одному файлу на подмножество Unicode (`cyrillic`, `cyrillic-ext`, `latin`, `latin-ext`) и подключается четырьмя `@font-face`. Референс нарисован в Gilroy: он лицензируется по местам и в репозитории лежать не может, а обычная его замена Poppins не покрывает кириллицу, без которой русский интерфейс не живёт. Шрифт хостим у себя, а не тянем с `fonts.gstatic.com`: на один origin в CSP и один round trip на холодном WebView меньше.
...
|---|---|
| `Button.svelte` | `variant: 'primary' \| 'ghost' \| 'danger' \| 'contrast'`, `size: 'sm' \| 'md'`, `loading`, `disabled`, `onclick`, `children` |
| `Card.svelte` | `padded`, `interactive`, `tone: 'surface' \| 'accent'`, `children` |
| `Input.svelte` | `value = $bindable()`, `label`, `error`, `type`, `maxlength` |
| `Textarea.svelte` | `value = $bindable()`, `label`, `error`, `maxlength`, `counter` |
| `Badge.svelte` | `tone: 'neutral' \| 'success' \| 'warn' \| 'danger' \| 'contrast'`, `children` |
...
Три тона рассчитаны друг на друга и порознь не работают: `Card tone="accent"` внутри себя требует `Button variant="contrast"` и `Badge tone="contrast"` — обычные `primary` и `success` на акцентной заливке становятся акцентом по акценту. Обратное тоже верно: `contrast` вне акцентной карточки сливается со страницей.

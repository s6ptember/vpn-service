...
| `subscription.notify_expiry` | sweep | `expiry:<subscriptionId>:<daysLeft>` | одно сообщение о скором окончании |
...
- **Главная**: карточки активных тарифов, отсортированы `sortOrder`. На карточке: имя, срок, цена, кнопка покупки. Если подписка активна — на карточке 30/90 дней подпись «продлит до <дата>».
...
- **Профиль**: аватар (`photoUrl`, фолбэк — инициалы), имя, `@username`. Ниже блок промокода. Ниже активный тариф: дата окончания, дней осталось, ссылка подписки с кнопкой «Скопировать» и QR. Ниже история покупок из `orders`. Для `ADMIN_CHAT_ID` — вход в админку.
...
| CI/CD, Caddyfile, compose | тимлид | |
